...
//	
	 ...
	 * @return
	 */
	public Long exist(ReqFrom reqFrom, String ouerUserId, String ouerShopId, String itemId) {
		return this.itemDao.exist(reqFrom, ouerUserId, ouerShopId, itemId);
	}
	...
	public void save(Item item, List<Sku> skus, List<Img> imgs) {
		// check
		final long exist = this.existCache(item.getReqFrom(),
				item.getOuerUserId(), item.getOuerShopId(), item.getShopType(),
				item.getItemId());
		if (exist != 0) {
		...
		// double check
		// 查询数据库判断
		final Long id = this.exist(item.getReqFrom(), item.getOuerUserId(),
				item.getOuerShopId(), item.getItemId());
		...
		if (id == null) {
			try {
				...
				Logs.unpredictableLogger.error(
						"Failed to insert item[{}],skus:[{}],imgs:[{}]",
						JSON.toJSONString(item), JSON.toJSONString(skus),
						JSON.toJSONString(imgs), e);
				return;
			}
		}
		// 更新
			...
			try {
				...
				Logs.unpredictableLogger.error(
						"Failed to update item[{}],skus:[{}],imgs:[{}]",
						JSON.toJSONString(item), JSON.toJSONString(skus),
						JSON.toJSONString(imgs), e);
				return;
			}
		...
		try {
			this.syncResultService.syncItem(item, skus, imgs);
		}
