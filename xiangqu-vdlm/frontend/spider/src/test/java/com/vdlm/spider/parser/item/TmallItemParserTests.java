...
public class TmallItemParserTests {

	private static final Logger LOG = LoggerFactory.getLogger(TmallItemParserTests.class);
	...
	public void before() {
		...
		this.testObject = new TmallItemParser(provider, bean, itemService) {
			...
			String getRemoteSkuInfo(String ajaxUrl) {
				try {
					final String file = this.getClass().getResource("/data/tmall.ajax.txt").getFile();
