    ...
    Wookmark = (function() {
        ...
        Wookmark.prototype.updateFilterClasses = function() {
            ...
            for (; i < this.handler.length; i++) {
                ...
                if (typeof itemFilterClasses == 'object' && itemFilterClasses && itemFilterClasses.length > 0) {
        ...
        Wookmark.prototype.layout = function(force) {
            ...
            if(window.af){
                if (this.container.isHide()) return;
            }else{
                if (!this.container.is(':visible')) return;
            }
            ...
            if (this.itemHeightsDirty || !this.container.data('itemHeightsInitialized')) {
                //var oldheight = this.container.find('img').attr('imgheight')+ $("pro-item-text")[0].clientHeight;
                //var oldwidth = this.container.imgobj.attr('imgwidth');
              /* this.container.find('img').each(function(i,obj){
                   $(obj).attr('imgheight');
                   console.log($(obj).attr('imgheight'));
               });*/
                for (; i < activeItemsLength; i++) {
                    $item = activeItems.eq(i);
                    var oldHeight=parseInt(this.container.find('img').eq(i).attr('imgheight'));
                    var oldWidth=parseInt(this.container.find('img').eq(i).attr('imgwidth'));
                    //$item.data('wookmark-height', $item.outerHeight());
                    if(parseInt(oldHeight*this.itemWidth/oldWidth)+parseInt($("."+this.proItemText)[i].offsetHeight)==NaN){
                        $item.data('wookmark-height', 0);
                    }else{
                        $item.data('wookmark-height', parseInt(oldHeight*this.itemWidth/oldWidth)+parseInt($("."+this.proItemText)[i].offsetHeight));
        ...
        Wookmark.prototype.layoutFull = function(columnWidth, columns, offset) {
            ...
            var arrSrc = this.getActiveItems();
            var arrTemp = [];

            for (var i = 0; i < arrSrc.length; i++) {
                arrTemp.push(arrSrc[i]);
            }
            ...
            for (; i < length; i++) {
                ...
                heights[shortestIndex] += Number($item.data('wookmark-height')) + this.verticalOffset;
